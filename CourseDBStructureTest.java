...
public class CourseDBStructureTest {
	...
	public void testHashTable()
	{
		...
		cds.add(cde);
		LinkedList<CourseDBElement> list = cds.hashTable[String.valueOf(cde.crn).hashCode()%cds.getTableSize()];
		assertEquals(39999, list.get(0).getCRN());
		
		cds = new CourseDBStructure("Testing",20);
		assertEquals(20, cds.hashTable.length);	
		cds.add(cde);
		list = cds.hashTable[String.valueOf(cde.crn).hashCode()%20];  //Changed from cde.hashCode() b/c taking the hashcode of crn rather than cde
		assertEquals(39999, list.get(0).getCRN());
	}
}
